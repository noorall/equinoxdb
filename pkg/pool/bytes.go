...
type BytesSyncPool struct {
	pool sync.Pool
}

func NewBytesSyncPool() *BytesSyncPool {
	return &BytesSyncPool{}
}

func (p *BytesSyncPool) Get(size int) []byte {
	x := p.pool.Get()
	if x == nil {
		b := make([]byte, size)
		return b
...
func (p *BytesSyncPool) Put(buf []byte) {
	p.pool.Put(buf)
}
